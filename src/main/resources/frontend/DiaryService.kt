...
import calebzhou.expdl.utils.WordUtils
...
/**
 * Created  on 2023-05-17,8:23.
 */
@Service
class DiaryService {
    fun createDiaryOneDay(count: Int, company: String?, date: LocalDate): String{
        val formatter = DateTimeFormatter.ofPattern("yyyy年MM月dd日")
        val formattedDate = date.format(formatter)
        val dayOfWeek = date.dayOfWeek.getDisplayName(TextStyle.SHORT, Locale.CHINESE)
        val header = "$formattedDate 第${(count/2)+1}周 $dayOfWeek"
        val progLangTech = ProgLangTech.allTechs.random()
        val tech = progLangTech.techs.random()
        val sys = CommercialSystems.all.random()
        val bug = tech.bugs.entries.random()
        var str=  """
            <br>$header<br>
            ${todays.random()}${weathers.random()}。${busys.random()}，${impls.random()}。
            系统是${company}负责维护的${sys.name}${sys.examples.random()}，${sys.desc} 
             它${tech.desc.random()}。<br>
            有时系统会出现问题，例如${bug.key},${bug.value}。
           
            ${PersonalThink.rand()}<br>
            
        """.trimIndent()
            .replace("%progLang%",progLangTech.name)
            .replace("%tech%",tech.name)
            .replace("%company%",company?:"公司")
        str+="(${WordUtils.countChineseWords(str)}字)"
        return str
    }
    val todays = "今天，今日，本日，当天".split("，")
